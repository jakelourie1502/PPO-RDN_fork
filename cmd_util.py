...
from environments.frozen_lakeGym_Image import gridWorld
from .environments.config import Config as cfg
...
def make_lake_env(num_env, seed):
    def make_env(rank):
        def _thunk():
            env = gridWorld(cfg())
            env = Monitor(env, logger.get_dir() and os.path.join(logger.get_dir(), str(rank)), allow_early_resets=True)
            ...
            return env
        return _thunk
    return SubprocVecEnv([make_env(i + 42) for i in range(num_env)])
