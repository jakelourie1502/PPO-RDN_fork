...
MAPS = {
    ...
    "6x6Easy": ['SFHHFF', 'FFHFHF', 'FFFFFF', 'FHFFFH', 'FFFFHH', 'FFHFFG'],
    "6x6Medium1": ['SFHHFF', 'FFFFHF', 'FFHHFF', 'FFHFFF', 'FFFFHF', 'FFHFHG'],
    "6x6Medium2": ['SFFFFF', 'FFHFHF', 'FHHFFF', 'FHFFHF', 'FHFFHF', 'FHFFHG'],
    "6x6Hard": ['SFHHFF', 'FFHFHF','FFHFFF','FHFFFF', 'FHFHHF', 'FFFFHG'],
    "8x8Easy": ['SHFFFHFF','FFFHFHHF','HFFHFFFH','HFFFHFFF','FFFHHFHF','HFFFFFFF','FFFFFFFF','FHFFHFFG'],
    "8x8Medium1": ['SFFFHFHF','FHHFFFFF','FFFFFFHF','FHFFFFFF','FHFFHFFF','FHFHFFFF','FHFFFFHH','HHHFFFFG'],
    "8x8Medium2": ['SHFFFFHF','FFFHFFFF','FFHFHFFF','FFHFHHFF','FFFFFFFF','HHHFFFHH','FFFFHHHF','FFFFFFFG'],
    "8x8Hard": ['SFHFFFFF','FHFFFFFH','FHFFHFFH','FFFFHFHH','FFFHHFFF','FFHFFHFF','FFHFFHHF','FFFFFHHG']}
...
class FrozenLakeEnv(Env):
    ...
    ### Arguments
    ```
    gym.make('FrozenLake-v1', desc=None,map_name="6x4", is_slippery=True)
    ```
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, size=6, desc=None, map_name="6x6Medium2", is_slippery=False,max_steps = 60):
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(nrow, ncol, 1), dtype=np.uint8)
    ...
    def step(self, a):
        ...
        obs = np.array([self._map(str(x)) for x in self.desc.flatten()]) 
        obs[self.s] = 255
        obs = obs.reshape(self.size,self.size,1).astype(np.uint8)
        # print(int(s),obs, r, d, {"prob": p})
        
        if r == 0.5:
            print('in game', r)
        return obs, r, d, {"st": int(self.s),"r": r} 
    ...
    def _map(self, x):
        my_mapping = {"b'S'": 0, "b'H'": 50, "b'G'":100, "b'F'":0}
        return my_mapping[x]
    
    def render(self, mode="human"):
        
        desc = self.desc
        print(np.array([self._map(str(x)) for x in desc.flatten()]).reshape(self.size,self.size))
        # if mode == "ansi":
