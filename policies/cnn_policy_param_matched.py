...
def _normalize(x):
    ...
    return (x - mean) / tf.math.sqrt(var + eps)
...
class CnnPolicy(StochasticPolicy):
    ...
    def define_self_prediction_rew(self, convfeat, rep_size, enlargement):
        ...
        self.feat_var = tf.math.reduce_mean(tf.nn.moments(X_r, axes=[0])[1])
        self.max_feat = tf.math.reduce_max(tf.abs(X_r))
        self.int_rew = tf.math.reduce_mean(tf.math.square(tf.stop_gradient(X_r) - X_r_hat), axis=-1, keepdims=True)
        ...
        self.aux_loss = tf.math.reduce_mean(tf.math.square(targets - X_r_hat), -1)
        ...
        self.aux_loss = tf.math.reduce_sum(mask * self.aux_loss) / tf.math.maximum(tf.math.reduce_sum(mask), 1.)
    ...
    def define_dynamics_prediction_rew(self, convfeat, rep_size, enlargement):
        ...
        self.feat_var = tf.math.reduce_mean(tf.nn.moments(X_r, axes=[0])[1])
        self.max_feat = tf.math.reduce_max(tf.math.abs(X_r))
        self.int_rew = tf.math.reduce_mean(tf.math.square(tf.stop_gradient(X_r) - X_r_hat), axis=-1, keep_dims=True)
        self.int_rew = tf.reshape(self.int_rew, (self.sy_nenvs, self.sy_nsteps - 1))

        noisy_targets = tf.stop_gradient(X_r)
        # self.aux_loss = tf.reduce_mean(tf.square(noisy_targets-X_r_hat))
        self.aux_loss = tf.math.reduce_mean(tf.math.square(noisy_targets - X_r_hat), -1)
