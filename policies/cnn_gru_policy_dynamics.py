...
class CnnGruPolicy(StochasticPolicy):
    ...
    def apply_policy(ph_ob, ph_new, ph_istate, reuse, scope, hidsize, memsize, extrahid, sy_nenvs, sy_nsteps, pdparamsize, rec_gate_init):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=reuse), tf.device('/gpu:0' if yes_gpu else '/cpu:0'):
            X = activ(conv(X, 'c1', nf=32, rf=1, stride=1, init_scale=np.sqrt(2), data_format=data_format))
            X = activ(conv(X, 'c2', nf=64, rf=1, stride=1, init_scale=np.sqrt(2), data_format=data_format))
            X = activ(conv(X, 'c3', nf=64, rf=1, stride=1, init_scale=np.sqrt(2), data_format=data_format))
            X = to2d(X)
    ...
    def define_self_prediction_rew(self, convfeat, rep_size, enlargement):
        ...
        for ph in self.ph_ob.values():
            if len(ph.shape.as_list()) == 5:  # B,T,H,W,C
                ...
                xr = tf.nn.leaky_relu(conv(xr, 'c1r', nf=convfeat * 1, rf=3, pad='SAME', stride=1, init_scale=np.sqrt(2)))
                xr = tf.nn.leaky_relu(conv(xr, 'c2r', nf=convfeat * 2 * 1, rf=1, stride=1, init_scale=np.sqrt(2)))
                xr = tf.nn.leaky_relu(conv(xr, 'c3r', nf=convfeat * 2 * 1, rf=1, stride=1, pad='SAME', init_scale=np.sqrt(2)))
                rgbr = [to2d(xr)]
        ...
        for ph in self.ph_ob.values():
            if len(ph.shape.as_list()) == 5:  # B,T,H,W,C
                ...
                xrp = tf.nn.leaky_relu(conv(xrp, 'c1rp_pred', nf=convfeat, rf=3,pad='SAME', stride=1, init_scale=np.sqrt(2)))
                xrp = tf.nn.leaky_relu(conv(xrp, 'c2rp_pred', nf=convfeat * 2, rf=1, stride=1, init_scale=np.sqrt(2)))
                xrp = tf.nn.leaky_relu(conv(xrp, 'c3rp_pred', nf=convfeat * 2, rf=3, pad='SAME',stride=1, init_scale=np.sqrt(2)))
                rgbrp = to2d(xrp)
        ...
        mask = tf.compat.v1.random_uniform(shape=tf.shape(self.aux_loss), minval=0., maxval=1., dtype=tf.float32)
