...
class PpoAgent(object):
    ...
    def __init__(self, *, scope,
        ...
        self.explorer_log = np.zeros((5*12 + 1))
        self.lr = lr
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            entropy = tf.math.reduce_mean(self.stochpol.pd_opt.entropy())
            vf_loss_int = (0.5 * vf_coef) * tf.math.reduce_mean(tf.math.square(self.stochpol.vpred_int_opt - self.ph_ret_int))
            vf_loss_ext = (0.5 * vf_coef) * tf.math.reduce_mean(tf.math.square(self.stochpol.vpred_ext_opt - self.ph_ret_ext))
            vf_loss = vf_loss_int + vf_loss_ext
            ratio = tf.math.exp(self.ph_oldnlp - neglogpac) # p_new / p_old
            negadv = - self.ph_adv
            ...
            approxkl = .5 * tf.math.reduce_mean(tf.math.square(neglogpac - self.ph_oldnlp))
            maxkl    = .5 * tf.math.reduce_max(tf.math.square(neglogpac - self.ph_oldnlp))
            clipfrac = tf.math.reduce_mean(tf.compat.v1.to_float(tf.math.greater(tf.math.abs(ratio - 1.0), self.ph_cliprange)))
            ...
            global_grad_norm = tf.linalg.global_norm(grads)
    ...
    def update(self):
        ...
        print(self.explorer_log[:-1].reshape(5,12))
        print(np.sum(self.explorer_log))
